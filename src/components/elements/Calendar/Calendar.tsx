...
const Calendar = ({ onChange, className, placeholder = 'From', ...props }: CalendarPropsTypes) => {
  ...
  const ExampleCustomInput = forwardRef(({ value, onChange, onClick, placeholder }: any, ref: any) => (
    <div className={styles.calendarInputContainer}>
      <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="#0F2978"
           className={classNames(styles.calendarIcon, {[styles.calendarIcon_filled]: value})}>
        <path
      ...
      <input
        className={classNames(styles.customInput, { [styles.customInput_filled]: value })} value={value} placeholder={placeholder}
        onClick={onClick} onChange={onChange} ref={ref} />
    </div>
  ));


  return (
    <DatePicker className={className} placeholderText={placeholder} selected={startDate} onChange={onChangeHandler} calendarContainer={MyContainer}
